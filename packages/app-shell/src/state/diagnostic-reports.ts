 ...
 */
export type ReportFailure = "disabled" | "forbidden" | "rejected" | "offline" | "unavailable";

export interface BenchReportEnvelope {
  reportId: string;
  kind: "bench";
  appVersion: string;
...
/** Отчёт стресс-теста большой, а мобильная сеть медленная: ждём дольше, чем события. */
