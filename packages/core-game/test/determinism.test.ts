...
describe("симуляция", () => {
  ...
  it("совпадает с эталонной контрольной суммой забега", () => {
    ...
    // Эталон обновлён вместе с балансом оружия. Убийств стало меньше — 685
    // против 956, — а уровень выше: 21 против 17. Оружие быстрее выносит
    // дорогих врагов, и та же минута приносит больше опыта с меньшего числа
    // трупов.
    expect(run.checksum).toBe(-522494804);
    expect(run.world.stats.enemiesKilled).toBe(685);
    expect(run.world.progression.level).toBe(21);
  });
