...
describe("симуляция", () => {
  ...
  it("совпадает с эталонной контрольной суммой забега", () => {
    ...
    // Эталон обновлён вместе с пробегающим роем: нетопыри встали в смесь
    // паттернов, и минутный прогон теперь убивает 431 против 311 — рой сам
    // приходит под оружие, — а уровень берёт 18-й вместо 15-го.
    expect(run.checksum).toBe(-487386287);
