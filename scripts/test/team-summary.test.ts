...
describe("подпись генератора в конце описания", () => {
  ...
  it("хвост агента целиком — подпись, ссылка на сессию и подпись интеграции под чертой — не уезжает", () => {
    const tail = [
      "🤖 Generated with [Claude Code](https://claude.com/claude-code)",
      "",
      "https://claude.ai/code/session_01EovDB2yD87M4Nk7cyXsrhA",
      "",
      "---",
      "_Generated by [Claude Code](https://claude.ai/code/session_01EovDB2yD87M4Nk7cyXsrhA)_",
    ].join("\n");
    expect(extractSection(`${TEAM}\n\n${tail}`, TEAM_SECTION)).toBe("📋 **СВОДКА**\n\nтекст сводки");
    // Только подпись интеграции под чертой — тоже.
    expect(extractSection(`${TEAM}\n\n---\n_Generated by [Claude Code](https://claude.ai/code)_`, TEAM_SECTION)).toBe("📋 **СВОДКА**\n\nтекст сводки");
  });
