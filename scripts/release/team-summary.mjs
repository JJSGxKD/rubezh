...
const GENERATOR_SIGNATURES = [
  /^🤖 Generated with\b/u,
  // Подпись, которую GitHub-интеграция агента дописывает под чертой в самый
  // конец описания, и ссылка на его сессию: без них сводка уезжала в чат
  // вместе с хвостом «Generated by Claude Code».
  /^_Generated by \[Claude Code\]\(/u,
  /^https:\/\/claude\.ai\/code\/session_[A-Za-z0-9]+$/u,
];
const HORIZONTAL_RULE = /^(?:-{3,}|\*{3,}|_{3,})$/;

/**
 * Текст без подписи генератора в хвосте — вместе с пустыми строками и чертой
 * `---` перед ней, которую ставят, чтобы отделить подпись. Подписей бывает
 * несколько подряд, и черта может стоять между ними — отрезаются все. Такая же
 * строка посреди текста остаётся: отрезается только хвост. Подписи нет — текст
...
function withoutGeneratorSignature(text) {
  ...
  while (end > 0) {
    ...
    if (line === "") {
      ...
      signed = true;
    } else if (signed && HORIZONTAL_RULE.test(line)) {
      end--;
    } else {
      break;
    }
