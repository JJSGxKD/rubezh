...
# Кавычки вокруг значения снимаются: compose снимает их сам, а скрипты
# бэкапа и выката подставляют значения в URL и команды как есть.
value() { grep -E "^$2=" "$1" 2>/dev/null | tail -1 | cut -d= -f2- | sed -E "s/^\"(.*)\"\$/\1/; s/^'(.*)'\$/\1/"; }
