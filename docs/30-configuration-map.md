...
### 1.1 Курсы валют и цены (`packages/fx`)

Ядро курсов и слой цен этапа 4 (`35-stage4-plan.md` §3.12, устройство —
`36-fx-rates-and-pricing.md`). Числа ниже — **рабочие значения** (Р31):
меняются здесь до появления редактора в панели (WP17), а потом — там.

| Что меняю | Где | Ломает |
|---|---|---|
| Валюты: код, вид, разрядность, прежние коды, сеть, площадка | `packages/fx/src/currency.ts` → `CURRENCIES`, `CURRENCY_CODES` | тест реестра валют, кодек снимка |
| Источники курсов: адреса, идентификаторы валют у источника, природный интервал, лимиты бесплатного и платного тарифа, заголовок ключа | `packages/fx/src/rates/sources/<источник>.ts`; порядок приоритета — `sources/index.ts` → `DEFAULT_SOURCES` | тесты источников на заготовленных ответах |
| Порог скачка и порог подтверждения вторым источником | `packages/fx/src/rates/checks.ts` → `DEFAULT_SPIKE_RULES` | тест проверки скачка |
| Через сколько курс считается устаревшим — по виду валюты | `packages/fx/src/rates/freshness.ts` → `DEFAULT_FRESHNESS` | тест свежести |
| Запас месячного бюджета, пол интервала, пауза после 429 и её потолок | `packages/fx/src/rates/planner.ts` → `DEFAULT_PLANNER_RULES` | тест планировщика |
| Какие валюты собираем, приоритет фиатных источников, таймаут запроса, срок лока | `packages/fx/src/rates/collector.ts` → `DEFAULT_COLLECT_RULES` | тест сбора |
| Правило округления цены по валюте: целые звёзды, «…9/…99» у рубля, два знака у Gram | `packages/fx/src/pricing/rounding.ts` → `DEFAULT_ROUNDING` | тест округления, цены в тесте прайс-листа |
| Срок пересчёта прайс-листа, скачок для внепланового пересчёта, сколько можно продавать по устаревшему курсу | `packages/fx/src/pricing/price-list.ts` → `DEFAULT_PRICING_RULES` | тест прайс-листа |
| Способы оплаты: площадка, провайдер, валюта, пределы, комиссия, доля площадки, страны, порядок | `packages/fx/src/pricing/payment-method.ts` → `DEFAULT_PAYMENT_METHODS` | тест реестра способов |
| Ценовые ступени в долларах | `packages/fx/src/pricing/product.ts` → `PRICE_TIERS` | тест товаров |
| Заданные курсы звёзд — цена и выручка — со сроком годности | пока данные модуля бэкенда (WP9); правка — в панели с аудитом (WP17) | — |
