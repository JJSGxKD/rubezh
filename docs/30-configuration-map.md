...
|---|---|---|
| Цвета, типографика, радиусы, тени, базовые кривые и длительности — общие для игры и панели | `packages/design-tokens/src/tokens.css` → `@theme`, `:root` | напарник |
| Слои, отступы площадки, анимации игры и гайдбука, кривая колеса | `packages/app-shell/src/design-system/tokens.css` → `@theme`, `:root` | напарник |
| Составные поверхности: фон приложения, карточки, панели, кнопки, заливки полос, ореолы | `tokens.css` → блоки `@utility` | напарник |
| Те же значения числами для канвы и графиков, имена гарнитур | `packages/design-tokens/src/index.ts` → `COLORS`, `DURATION`, `FONT_FAMILY` | напарник |
| Цвета шапки и панели Telegram | `app-shell/src/design-system/tokens.ts` → `PLATFORM_COLORS` | напарник |
...
| Снаряжение: базы слотов и пулы свойств, база каждого свойства, правила редкостей — множитель значений, число свойств, множитель цен, осколки разбора (Р38) | `backend/api/src/modules/items/item-catalog.ts` → `SLOT_BASES`, `STAT_BASE`, `RARITY_RULES` | геймдизайнер |
| Снаряжение: предел и шаг уровня предмета, диапазоны бросков, потолок инвентаря, поправка уровня добычи за сложность | `backend/api/src/modules/items/item-catalog.ts` → `MAX_ITEM_LEVEL`, `LEVEL_STEP`, `MAIN_ROLL`, `EXTRA_ROLL`, `INVENTORY_CAP`, `DIFFICULTY_LEVEL_BONUS` | геймдизайнер |
| Снаряжение: шанс и веса редкостей добычи забега, до какой редкости собирает объединение и сколько предметов в него идёт | `backend/api/src/modules/items/item-catalog.ts` → `LOOT`, `MAX_MERGE_RESULT`, `MERGE_COUNT` | геймдизайнер |
| Снаряжение: цены улучшения, перековки и объединения, осколки разбора, потолок уровня предметов от уровня аккаунта, уровень выпавшего | `backend/api/src/modules/items/item-rules.ts` → `upgradeCost`, `rerollCost`, `mergeCost`, `salvageYield`, `levelCap`, `dropLevel` | геймдизайнер, участник 1 |
| Снаряжение: лимиты частоты чтения инвентаря и операций над предметами | `backend/api/src/modules/items/items-limits.ts` → `ITEM_LIMITS` | участник 1 |
...
| Рефералка: окно возраста аккаунта для привязки, сколько забегов — активация, монеты приглашённому и пригласившему, потолок активаций в сутки — рабочие (О16, Р31); сколько сессий пригласившего сверяется по подсети | `backend/api/src/modules/referrals/referral-rules.ts` → `REFERRAL_RULES`; страховка — `WALLET_DAILY_CAPS.referral_reward` | участник 1 |
| Возвращение: после скольких суток без входа игрок ушедший, сколько ждём забега, монеты каждому из пары, период на пару — рабочие (О15, Р31) | `backend/api/src/modules/referrals/referral-rules.ts` → `RETURN_RULES` | участник 1 |
| Флаги функций: включён ли, площадки, доля игроков — данными в базе, из панели (раздел «Флаги»), не кодом; формат ключа, срок кеша правил на реплике, лимит чтения игрой | панель → «Флаги»; `backend/api/src/modules/flags/flag-rollout.ts` → `FLAG_KEY`; `flags.service.ts` → `CACHE_MS`; `flags.controller.ts` → `READ_LIMIT` | владелец, администратор |
| Редирект-ссылки: текст превью (заголовок, описание), список краулеров превью, лимит переходов с адреса | `backend/api/src/modules/links/link-page.ts` → `LINK_PAGE`; `crawler.ts` → `CRAWLERS`; `redirect.controller.ts` → `VISITS_PER_MINUTE` | участник 1 |
| Бонус за число друзей: пороги ступеней и монеты каждой — рабочие (О15, Р31); последняя ступень — потолок | `backend/api/src/modules/friends/friends-rules.ts` → `FRIEND_BONUS_RULES`; страховка — `WALLET_DAILY_CAPS.friend_bonus` | участник 1 |
...
| Прод: сервисы, лимиты памяти, образы, сети | `infra/prod/compose.yml` | участник 1 |
| Прод: домены, прокси, кэширование статики, туннели | `infra/prod/Caddyfile`, `infra/prod/frps.toml` | участник 1 |
...
| Аналитика Graspil в Telegram-сборке: ключ проекта, домены в политике источников | переменная репозитория `GRASPIL_KEY` → `VITE_GRASPIL_KEY`; `scripts/vite/content-security-policy.ts` → `GRASPIL_*_ORIGINS` | участник 1 |
...
и `index.ts` пакета токенов обязаны совпадать** — расхождение ловит
`design-tokens/test/tokens.test.ts`.
...
| `bh.loadout.v1` | подписанный снимок надетого для следующего забега — забег без сети начинается с ним; пустое снаряжение не хранится | `state/run-loadouts.ts` |
| `bh.run-loadouts.v1` | с каким снимком начался каждый забег — итог несёт его серверу; до 20 записей, старые вытесняются | `state/run-loadouts.ts` |
