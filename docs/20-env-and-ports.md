...
| 7. Платформы | токены ботов, чтение обновлений бота (`TELEGRAM_BOT_UPDATES`), чаты администраторов (`ADMIN_CHAT_ID` и адреса потоков `ADMIN_CHAT_STATS`, `ADMIN_CHAT_STRESS`, `ADMIN_CHAT_RUNS`) и уведомления в них (`ADMIN_NOTIFY_REPORTS`), секрет вебхука бота (`TELEGRAM_WEBHOOK_SECRET`, с этапа 2), ключи Bridge, OAuth | из кабинетов площадок; для staging — **отдельный** бот; секрет вебхука генерируется. `PLAYTEST_STATS_CHAT_ID` переименована в `ADMIN_CHAT_ID` — со старым именем бэкенд не стартует и называет новое |
