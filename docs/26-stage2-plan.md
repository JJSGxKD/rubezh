...
- [ ] Визуальное направление: палитра, шрифты, стиль иконок и карточек под
      сеттинг (участник 2 по направлению участника 3). **Не сделано и не
      блокирует**: каркас собран на нейтральных токенах-плейсхолдерах, смена
      направления — правка `tokens.css` и `tokens.ts`, а не всех экранов
...
- **Бюджет первой загрузки не сходится с §3.4.** Документ закладывал 150 КБ
  gzip исходя из оценки React в ~60 КБ. Замеренная сборка: **175 КБ** —
  React 19 вместе с `react-dom` весит заметно больше оценки, и на неё же
  приходится почти весь объём. Бюджет в `scripts/bundle-budget.mjs`
  выставлен по факту (190 КБ) и дальше расти не даёт. Это не «подвинули
  число, раз покраснело»: решение Р1 принималось с другой арифметикой, и
  пересматривать его — дело команды, а не этого PR
  (`27-design-system-and-app-shell.md` §1.3 и §3.4).
