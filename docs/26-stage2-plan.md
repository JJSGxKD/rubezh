...
| Звук на файлах-ассетах и музыка | По готовности ассетов | Звук процедурный и работает; музыка удалена до переработки (`31-audio-and-haptics.md` §5) |
...
- [ ] Прод- и staging-compose: API, Postgres, Redis, файлсервер статики; порты
      БД, Redis и API не публикуются. Postgres и Redis — те же версия и
      digest, что в `docker-compose.yml` и CI (`16-tech-stack-decisions.md`
      §9.4). **Отдельного контейнера воркера очередей нет**:
      ...
      появится вместе с несколькими репликами API. Воркер симуляции этапа 4
      (`35-stage4-plan.md`, Р15) — другое: у него своя, процессорная работа
...
- [x] Actions — по SHA (`09-ci-cd.md` §12)
...
- [x] **Сохранения и лидерборд на бэкенде** (Р19): модуль `playtest` —
      ...
      сознательно: данные теста стираются (`21-diagrams.md` §4.11). На
      этапе 3 заменено забегами под аккаунтом (`34-stage3-plan.md`, WP4)
...
- [x] **Звук:** процедурные звуки с правилами грани, звук интерфейса,
      регуляторы громкости, звуковая лаборатория у тестировщиков
      (`31-audio-and-haptics.md`). Музыка была и удалена: однообразная, она
      надоедала к третьему забегу и звучала даже при нулевой громкости —
      возвращать её начнём с замысла, а не с того кода (§5 там же)
...
Этап 3 закрыт и выпущен в `v0.5.0`, поэтому закрытый тест выйдет уже с
аккаунтами, рейтингом под ними и вторым шансом за звёзды. Выпускается
стабильный тег `main`, актуальный на тот момент. Здесь же проходится
чек-лист приёмки этапа 3 (`34-stage3-plan.md` §8.2): на сервере он проверяет
больше, чем на машине разработчика.
