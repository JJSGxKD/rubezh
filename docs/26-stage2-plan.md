...
> **Статус: не реализовано. Обязательно к релизу закрытого теста** (§6).
> Переключатель «Запись забегов» уже есть и включается вместе с режимом
> диагностики по умолчанию, но движок пока ничего не записывает — подсказка
> у переключателя говорит об этом прямо. Отложено сознательно при
> планировании WP14: разбор функционала вживую закрывает режим
> разработчика, а воспроизведение бага тестера без записи — нет.
...
- [ ] **Запись забегов работает** (WP7): отчёт тестера доходит, повтор по нему
      даёт тот же исход — отложено при WP14, не забыть
