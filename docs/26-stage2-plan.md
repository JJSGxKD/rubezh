...
| Платежи Stars, континью | Этап 3 | Нечего продавать до проверки геймплея |
...
| Мета-прогрессия, забег-валюта | Этапы 4–5 | Разделы есть в UI, но только заглушками |
...
| Р13 | Слоты и потолки прокачки | **4 оружия + 4 пассивных улучшения**, у каждого 5–8 уровней. У Vampire Survivors 6 + 6, но на экране телефона 12 иконок в HUD и 12 вариантов в пуле делают выбор нечитаемым |
...
- [ ] Спецификация четырёх новых паттернов: что делает, как телеграфирует,
      какие параметры хочет крутить (участник 3)
- [ ] Возможности паттерна — данными, а не проверкой имени: сейчас
      `step.ts` пропускает контактный урон условием
      `pattern === "kite_and_shoot"`. Нужна таблица возможностей паттерна
      (`contactDamage`, радиус) рядом с `RADIUS_BY_PATTERN`
      (`15-engineering-standards.md` §1, принцип 4)
- [ ] Параметры паттернов, которые крутит геймдизайнер (дистанция стрелка,
      время телеграфа рывка, радиус взрыва), — необязательные поля `EnemyDef`
      с дефолтами в паттерне
- [ ] Ценность опыта врага — поле `EnemyDef` (WP2)
- [ ] Реализация `dash`, `orbit`, `exploder`, `splitter` в `game/patterns/`
- [ ] Состояние паттерна (фаза телеграфа, таймер рывка) — в SoA-пулах
      `World`, без объектов на врага
- [ ] `splitter` спавнит через общий `spawnEnemy` с учётом ёмкости пула;
      при исчерпании пула — детерминированно не спавнит, а не падает
- [ ] Паттерны не выводят врагов за границы карты (WP4.2)
- [ ] Эффект взрыва — событие симуляции, которое рендер рисует
- [ ] Визуал врагов и телеграфов: плейсхолдеры разной формы и цвета до
      прихода ассетов (участник 2)
- [ ] Заведение типов в `content/enemies.ts` (участник 3, скилл `add-content`)
...
**Тесты** (`17-testing-strategy.md` §3): поведенческий тест на каждый паттерн
в `sim-behaviour.test.ts`; `sim-purity.test.ts` покрывает новые файлы;
тест контента на границы параметров; `perf-budget.test.ts` на смешанной
популяции со всеми паттернами.

**Аналитика:** `run_finished.payload.killsByEnemy`,
`run_finished.payload.deathCause` — какой враг чаще всего убивает.
...
- [ ] Спецификация трёх стартовых оружий, пула оружий и пассивок под слоты
      4 + 4, кривой опыта (участник 3)
- [ ] Модель `WeaponDef`, пассивок и эффектов в `shared-types` (участник 1)
- [ ] Перевод текущей атаки в оружие `projectile_nearest` без изменения
      поведения — golden-прогоны до и после совпадают
- [ ] Поведения оружия в `game/weapons/*` (участник 1)
- [ ] Пул кристаллов опыта — SoA, как враги и снаряды; радиус притяжения;
      **слияние кристаллов** при превышении потолка на экране (как в Vampire
      Survivors), чтобы опыт не стал третьей осью нагрузки
- [ ] Кривая опыта, очередь уровней, генерация предложений, применение выбора
- [ ] Экран выбора — React-оверлей над остановленной канвой: три карточки,
      текущий и следующий уровень, что изменится
      (`27-design-system-and-app-shell.md` §3)
- [ ] Экран выбора стартового оружия перед забегом, последний выбор
      запоминается
- [ ] Тексты оружий и улучшений — ключами i18n, а не русскими строками в
      контенте, как сейчас в `upgrades.ts` (`01-tech-stack.md` §7)
...
- [ ] Сбор статистики в симуляции — счётчиками в `World`, без аллокаций в кадре
- [ ] Экран смерти (React): итоги, оружие с уроном, кнопки «Ещё раз» и «В меню»;
      «Поделиться» — видимая заглушка «скоро»
- [ ] «Ещё раз» не перезагружает Phaser: новая сцена в загруженном движке,
      новый seed, то же стартовое оружие
- [ ] В режиме диагностики на экране смерти виден seed и `runId`
- [ ] Локальный рекорд — в хранилище устройства
      (`27-design-system-and-app-shell.md` §7)
- [ ] Экран паузы: продолжить, звук и вибрация, сдаться с подтверждением.
...
`28-diagnostics.md` §3.2), `run_abandoned`, `run_paused`.
...
- [ ] Координаты без границ. Позиции в пулах — **`Float64Array`**: за
...
- [ ] Сетка коллизий вокруг игрока, а не под размер мира: хеш-сетка или
      сетка, перестраиваемая относительно позиции игрока
- [ ] Враги дальше радиуса удержания не копятся позади: переносятся на
      кольцо спавна **впереди** по направлению движения. Давление
      сохраняется, пул не забивается потерянными врагами. Детерминированно
- [ ] Снаряды и кристаллы вне радиуса — по времени жизни и дистанции
- [ ] Фон — повторяющаяся текстура, движется с камерой (рендер)

#### WP4.2 Границы карты — особенность карты

- [ ] Карта — данные в `content/maps.ts` (новый файл контента, одна карта):
      необязательные `bounds` — по ширине, по высоте, обе или ни одной
- [ ] Игрок и враги не выходят за границы, снаряды на границе гибнут
- [ ] **Спавн никогда за границей**: из кольца спавна выбираются только
      допустимые дуги; если коридор уже видимой области — спавн вдоль
      открытой оси за краем видимости
- [ ] С границами или без — решает геймдизайнер для единственной карты

#### WP4.3 Динамическая камера

- [ ] Камера следует за игроком со сглаживанием, без жёсткой привязки
- [ ] **Масштаб от движения**: стоит — ближе, идёт — дальше. Переход
      ...
      дёргать камеру
- [ ] Параметры (ближний и дальний масштаб, время перехода, задержка) — в
...
- [ ] **Камера — только рендер.** Симуляция о масштабе не знает, детерминизм
      не затронут
- [ ] **Спавн считается от максимальной видимой области** — дальний масштаб,
...
- [ ] Таймлайн из отрезков (по умолчанию — минута): состав врагов, темп
      спавна, потолок живых
- [ ] Элиты и мини-боссы в заданные минуты; события таймлайна — «окружение
      кольцом», «рой с одной стороны»
- [ ] Первые N минут — вручную в `waves.ts`; дальше — генерация по
...
- [ ] Кривые опыта и сложности калибруются вместе: темп прокачки обязан
      успевать за угрозой, иначе после десятой минуты забег решает
      не выбор, а арифметика
- [ ] В аналитике «волна» — это отрезок таймлайна (`wave_reached`): термин
...
- [ ] В `game/sim/*`, `game/patterns/*`, `game/weapons/*`: `Math.hypot` →
...
- [ ] `sim-purity.test.ts` запрещает эти функции в симуляции так же, как
      `Math.random` и `Date.now`
- [ ] Тест: эталонный забег, снятый на Android, повторяется в CI на Node
      (V8) и совпадает; прогон того же эталона на iOS — пункт ручного QA
- [ ] Рендер и камера (`Math.exp` в сглаживании) — вне требования: на
      исход забега они не влияют

#### WP4.6 Калибровка (участник 3 при поддержке участника 1)

- [ ] Целевые коридоры фиксируются **до** калибровки: медиана выживания
...
- [ ] Бот-игрок на основе автопилота стенда (`game/bench/autopilot.ts`): уровни
      «пассивный» и «уклоняющийся», выбор улучшений по простой стратегии
- [ ] `pnpm balance:sim`: N seed × сценарии × стартовые оружия →
...
- [ ] Пакет `packages/app-shell`: React-оболочка, дизайн-система, экраны,
      состояние; правила импортов — линтом границ или тестом по исходникам
      (участник 1)
- [ ] Контракт «движок ↔ оболочка» и ленивая загрузка Phaser (участник 1)
- [ ] Платформенные возможности интерфейса в `PlatformAdapter`: кнопка
...
- [ ] **Выбор режима экрана**: настройка «Полноэкранный режим», применяется
...
- [ ] **Любая ориентация**: портретная и ландшафтная раскладки всех экранов,
      в том числе ландшафт на телефоне с высотой около 360 CSS px
      (`27-design-system-and-app-shell.md` §5.3)
- [ ] Визуальное направление: палитра, шрифты, стиль иконок и карточек под
      сеттинг (участник 2 по направлению участника 3)
- [ ] Токены и базовые компоненты (участник 1 + 2)
- [ ] Навигация: стек экранов, панель разделов, кнопка «назад» Telegram
- [ ] Экраны по карте (`27-design-system-and-app-shell.md` §6): рабочие —
      загрузка, главная, выбор оружия, забег, пауза, выбор улучшения, смерть,
      настройки, диагностика; заглушки — остальные разделы
- [ ] Экран «открыто вне Telegram» — внятный, а не белый
- [ ] HUD: HP, таймер, уровень и полоса опыта, слоты оружия и пассивок; пауза
- [ ] Плавающий виртуальный джойстик — появляется под пальцем
- [ ] Витрина компонентов в режиме диагностики
- [ ] Бюджет бандла объявлен и проверяется в CI (`09-ci-cd.md` §2, шаг 7)
...
- [ ] **Выгрузка для анализа** (Р11) — один сервис сборки архива за период:
  ...
  - `platform_user_id` в выгрузке **псевдонимизирован** (хэш с солью,
    соль — секрет окружения): выгрузка уходит во внешний сервис ИИ, а
    Telegram ID — персональные данные
