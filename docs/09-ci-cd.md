...
|---|---|---|
| `ci.yml` | PR, push в `main` и `dev` | Гейт §2 и сборка образа бэкенда без публикации; на push в `main` и `dev` — джоб релиза §8.1: версия (из `dev` — предрелиз), образ в GHCR, тег, GitHub Release с архивом статики. Сборка матрицы §1 — ещё не реализована |
| `pr-checks.yml` | открытие и правка PR, смена меток | Заголовок PR по правилам коммитов и метка типа релиза (§8.1); у служебных PR между `main` и `dev` — только заголовок |
| `sync-dev.yml` | push в `main` | Возврат `main` в `dev`: перемотка или PR (§8.1, «Ветка dev») |
| `team-summary.yml` | влитый PR в `dev` или `main` | Сводка для команды и черновик поста в канал — из описания PR ботом в чат администраторов («Сводка в Telegram» ниже) |
...
`## Сводка для команды`, и проходит ревью вместе с кодом. При мердже
...
Необязательный раздел `## Пост в канал` уходит туда же **черновиком**, с
закрывашкой канала. Публикует его человек — пересылкой в канал «без автора»:
оформление сохраняется целиком. Автопоста в публичный канал нет
сознательно: опечатка или лишняя подробность ушли бы сразу всем подписчикам,
а удалённый пост успевают прочитать.

Настройка — один раз, в **Settings → Environments** репозитория:

1. создать окружение `telegram`, в «Deployment branches» оставить только
   `main` и `dev`;
2. секрет окружения `TEAM_TELEGRAM_BOT_TOKEN` — токен бота, который состоит в
   чате администраторов. Годится и бот закрытого теста: отправка сообщений не
   мешает бэкенду читать обновления того же бота;
3. переменная окружения `TEAM_TELEGRAM_CHAT` — адрес чата: `id` или `id:тема`
   (та же запись, что у `ADMIN_CHAT_*` бэкенда, `20-env-and-ports.md` §3);
4. необязательная `TEAM_TELEGRAM_DRAFTS_CHAT` — отдельная тема для
   черновиков постов; не задана — черновики идут в тот же чат.

Без секрета джоб не падает, а пишет предупреждение в сводку прогона: так
выглядят и PR из форка, которым GitHub секретов не даёт. Разделов в описании
нет — отправлять нечего, это тоже не ошибка: служебные PR сводку не пишут.

Устроено так, чтобы текст PR не мог стать командой: событие `pull_request`, а
не `pull_request_target` — PR из форка секретов не получает; код PR не
собирается и не запускается, работает скрипт из базовой ветки, куда PR уже
влит; описание приходит переменной окружения, а не подстановкой в `run:`.
...
1. CI собирает Docker-образ (`backend/api/Dockerfile`), тегирует `sha` и
   семантической версией, пушит в **GHCR**. Git-тег ставится только после
   успешной публикации образа: тег, по которому нечего деплоить, хуже
   отсутствующего. Образ собирается и на каждом PR — без публикации, чтобы
   поломка Dockerfile не всплывала в момент релиза.
...
**Заголовки на краю** — работа Caddy, когда он появится (WP10, часть 2).
Две вещи, которые приложение за него сделать не может:
...
- **политика источников клиента** — строго та, что возвращает
  ...
  `frame-ancestors` пускает Telegram Web;
