...
1. **Заголовок** по правилам `CLAUDE.md`: тип из списка, необязательный
   ...
   При squash-merge заголовок становится сообщением коммита в `main`.
...
```mermaid
flowchart LR
    PR["PR<br/>заголовок и метка проверены"] --> SQ["squash-merge<br/>в main"]
    SQ --> GATE["гейт §2"]
...
- PR фичи в `dev` — squash, как сейчас в `main`. **Релизный PR `dev` → `main` —
  merge-коммит**: при squash история PR схлопнулась бы в один коммит, скрипт
  не нашёл бы метки, а обратные слияния `main` → `dev` начали бы
...
`main` — точка отсчёта; в настройках — squash-merge для PR в `main`.
Сейчас в истории `main` merge-коммиты («Merge pull request #3…») — до
включения проверок это расхождение с §8 уже есть.
