...
3. **Метка не ниже, чем следует из заголовка**: `feat` — не ниже `minor`;
   ...
   промолчал.
   **Исключение — `feat(dev)`: не ниже `patch`.** Инструменты разработчика до
   игрока не доходят, а версия у монорепо одна: без исключения правка
   dev-сервера двигала бы минорный номер наравне с новой механикой в игре, и
   он перестал бы что-либо значить. Список таких областей — `TOOLING_SCOPES` в
