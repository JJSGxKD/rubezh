...
- **сторонние actions пинуются по SHA**, не по тегу: тег перемещаемый, а это
  ...
  `scripts/action-pins.mjs` тестом в `pnpm test`: `uses:` по тегу, по ветке,
  без версии или в виде, которого проверка не разобрала, роняет гейт;
...
вместе и во всех workflow сразу. Новый мажор — отдельным PR с заметкой о
ломающих изменениях, как у любой зависимости (`16-tech-stack-decisions.md` §9).
