...
flowchart TD
    ...
    FX["packages/fx<br/>курсы валют и цены,<br/>собирается в JS"]
...
`packages/fx` — ядро курсов валют и цен (`35-stage4-plan.md` §3.12, Р32;
устройство — `36-fx-rates-and-pricing.md`): ни одной стрелки **из** него в
монорепо, только `zod`. Это первый пакет, который собирается в JS для бэкенда,
потому что собранный бэкенд не импортирует исходники пакетов. Пунктирная
стрелка — модуль бэкенда `modules/fx` этапа 4 (WP9), который придёт после
портов площадок (WP1); до него пакет живёт со своими тестами, а границы
проверяет `packages/fx/test/boundaries.test.ts`.
