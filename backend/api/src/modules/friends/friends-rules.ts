import type { RateLimit } from "../ingest/rate-limiter.js";

/**
 * Числа дружбы (docs/35-stage4-plan.md, О15). **Рабочие** (Р31): поставлены
 * сразу, чтобы механика работала, и меняются, когда появится редактор
 * баланса (WP17) или решение команды. Потолки — чтобы добавляться было
 * выгодно, а строить ферму аккаунтов — нет (§3.8).
 */
export const FRIENDS_RULES = {
  /** потолок друзей у аккаунта — рабочее */
  maxFriends: 100,
  /** сколько входящих заявок ждёт ответа одновременно — рабочее */
  maxIncomingRequests: 50,
  /** сколько своих заявок висит без ответа — рабочее */
  maxOutgoingRequests: 50,
} as const;

/**
 * Подарки друзьям (О15), **рабочие**: раз в игровые сутки каждому другу,
 * получатель забирает сам. Потолок забранного в сутки держит ферму аккаунтов
 * невыгодной: сто друзей не дают сто подарков в день. Суточный потолок
 * кошелька на `friend_gift` — страховка поверх (`wallet-limits.ts`).
 */
export const GIFT_RULES = {
  /** монет в одном подарке — рабочее */
  coins: 20,
  /** сколько подарков можно забрать за игровые сутки — рабочее */
  maxClaimsPerDay: 20,
  /** незабранный подарок старше этого сгорает: копить его месяцами незачем */
  maxAgeDays: 7,
} as const;

/**
 * Бонус за число друзей (О15), **рабочие**: ступени разовые — каждая один раз
 * на аккаунт, выше последней бонуса нет, это и есть потолок. Друг
 * засчитывается, только если сыграл хоть один честный забег: пустые аккаунты
 * ферме ничего не дают, а живой друг, приведённый в игру, — даёт.
 * Всего — 950 монет: заметно новичку, незаметно в экономике.
 */
export const FRIEND_BONUS_RULES = {
  steps: [
    { friends: 1, coins: 50 },
    { friends: 3, coins: 100 },
    { friends: 5, coins: 150 },
    { friends: 10, coins: 250 },
    { friends: 20, coins: 400 },
  ],
} as const;

/**
 * Лимиты частоты по аккаунту. Заявка — сообщение другому человеку, поэтому
 * её лимит суточный и тесный: иначе раздел друзей становится рассылкой.
 */
export const FRIENDS_LIMITS = {
  read: { scope: "friends:read", limit: 600, windowSec: 3600 },
  change: { scope: "friends:change", limit: 120, windowSec: 3600 },
  request: { scope: "friends:request", limit: 30, windowSec: 86_400 },
} as const satisfies Record<string, RateLimit>;
